...
# Check events
kubectl get events -n dclaw-waste --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-waste deployment/dclaw-waste-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-waste-db-credentials -n dclaw-waste
   ```
