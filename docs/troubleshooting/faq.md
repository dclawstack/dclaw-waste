...
### How do I update DClaw Waste?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp waste --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Waste without Kubernetes?

Yes, for local development:

```bash
cd dclaw-waste/frontend && npm run dev
cd dclaw-waste/backend && uvicorn dclaw_waste.main:app --reload
```
